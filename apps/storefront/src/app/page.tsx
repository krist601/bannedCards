...
import { isOutOfStock, orderCatalogue } from "@/application/catalogue";
import type { CatalogueItem, Collection } from "@/domain/commerce";
...
export default function Home() {
  ...
  const results = useMemo(() => orderCatalogue(cards.filter((card) => (`${card.name} ${card.set}`).toLowerCase().includes(query.toLowerCase()) && (collection === "All" || card.collection === collection))), [cards, collection, query]);
  ...
  return <main>
    <header className="site-header">
      ...
      <form className="header-search" role="search" onSubmit={event => { event.preventDefault(); document.getElementById("singles")?.scrollIntoView(); }}>
        ...
        <input id="card-search" type="search" value={query} onChange={event => { setQuery(event.target.value); setCollection("All"); }} placeholder="Search cards or expansions…" />

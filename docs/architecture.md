...
The shop requests `/store/tcg/cards?grouped=true&limit=40&offset=0` with optional `q`, `sets` (codes), and `ranked` (ordered variant IDs). The server filters/sorts before paging printing-language groups, returns their complete variant rows and supplies total `count` and `nextOffset`. The legacy ungrouped endpoint remains compatible.

`use-catalogue-pages.ts` debounces search, aborts stale requests and appends pages when the bottom sentinel approaches the viewport. Retry keeps loaded cards. Cart/session initialization no longer fetches the catalogue. Database reads currently scan/project catalogue batches per request; only the client transfer/rendering is incremental. Larger catalogues will need an indexed query for these group rankings.
